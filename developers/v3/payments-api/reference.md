...
This page is **auto-generated** from our OpenAPI specification at `../../openapi/v3/payments.yaml`. Every operation includes the full parameter list, response schema, and an interactive **Test it** panel.
...
```
<code class="expression">space.vars.docs_root</code>/openapi/v3/payments.yaml
```
...
## Sample: the create-payment operation

For reference, here's how `POST /payments` is defined in the spec — this is the kind of detail the rendered reference will surface for every operation, with a **Test it** panel attached.
...
paths:
  /payments:
    post:
      ...
      description: |
        Charge a payment method. `capture_method` defaults to `automatic` (immediate capture).
        Set to `manual` for two-step authorize-then-capture, or `automatic_async` for batched capture.
      tags: [payments]
      requestBody:
        required: true
        content:
