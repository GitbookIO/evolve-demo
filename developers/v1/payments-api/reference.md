...
This page is **auto-generated** from our OpenAPI specification at `../../openapi/v1/payments.yaml`. Every operation includes the full parameter list, response schema, and an interactive **Test it** panel.
...
## Sample: the v1 source-charge operation

For reference, here's how the legacy `POST /sources/charge` is defined in the v1 spec. v1 uses source-prefixed endpoints rather than the unified `/charges` resource that v2 introduced:
...
paths:
  /sources/charge:
    post:
      ...
      description: |
        Charge a card or bank-account source. Source type is inferred from the token prefix.
        Note: idempotency keys are accepted but not required in v1.
      tags: [charges]
      requestBody:
        required: true
        content:
...
* [Overview](README.md) — deprecation notice and migration guide.
